...
from .auto_scale_wa_distort import auto_scale_block_wa_distort
...
from qmllm.methods.vtpq.quantize.token_reducer import reduce_calibration_tokens

__all__ = ["run_vtpq", "apply_vtpq", "collect_vtpq_score_features"]
...
@torch.no_grad()
def collect_vtpq_score_features(model, prompt_inputs, prompt_kwargs, num_layers):
    """
    Collect hidden states before the first num_layers LLM blocks.

    These features are used only to compute calibration-time token scores; the
    reduced sequence is captured again afterward before the regular scale search.
    """
    ...
    if not inps:
        raise RuntimeError("VTPQ failed to capture the first-layer input for score features.")
    ...
    for layer_idx in range(num_layers):
        ...
        score_features.append(inps.detach().cpu())

        out = layer(inps, **cur_kwargs)
    ...
    model.to_cpu()
    print(f"[VTPQ] collected {len(score_features)} layer feature(s) for token scoring")
...
def run_vtpq(
    ...
    score_features = None
    if vtpq_score_source == "layer":
        score_features = collect_vtpq_score_features(
            model=model,
            prompt_inputs=prompt_inputs,
            prompt_kwargs=prompt_kwargs,
            num_layers=vtpq_score_layers,
        )

    prompt_inputs, prompt_kwargs, vtpq_info = reduce_calibration_tokens(
        prompt_inputs=prompt_inputs,
        prompt_kwargs=prompt_kwargs,
        keep_ratio=vtpq_keep_ratio,
        semantic_lambda=vtpq_lambda,
        score_bit=vtpq_score_bit,
        protect_front=vtpq_always_keep_front,
        score_features=score_features,
        positive_only=vtpq_positive_only,
        min_prune_score=vtpq_min_prune_score,
    )
    ...
    vtpq_results = {
        ...
        "vtpq": vtpq_info,
        "scale_search": {
            "loss": str(vtpq_scale_loss),
            "tail_ratio": float(vtpq_tail_ratio),
            "tail_weight": float(vtpq_tail_weight),
        },
    }
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="Running VTPQ..."):
        layer = layers[i]
        layer = layer.cuda()
        ...
        if (
            ...
            if wa_quant:
                scales_list = auto_scale_block_wa_distort(
                    ...
                    loss_mode=loss_mode,
                    scale_loss=vtpq_scale_loss,
                    tail_ratio=vtpq_tail_ratio,
                    tail_weight=vtpq_tail_weight,
                )
            else:
            ...
            vtpq_results["scale"] += append_str_prefix(
...
def apply_vtpq(model, vtpq_results):
    apply_scale(model, vtpq_results["scale"])
