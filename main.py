...
from qmllm.methods.vtpq.prefill_profiler import (
    PREFILL_PROFILE_MODELS,
    install_prefill_profiler,
)
...
def parse_eval_args() -> argparse.Namespace:
    ...
    parser.add_argument("--method", default="awq", choices=["awq", "smoothquant", "mbq", "vtpq", "rtn", "gptq", "igptq", None])
    ...
    vtpq_infer_gate_group.add_argument(
        ...
        help=(
            ...
            "drop only visual tokens with positive predicted quantization gain "
            "and prune score above vtpq_infer_min_score."
        ),
    )
    vtpq_infer_gate_group.add_argument(
    ...
    parser.add_argument(
        "--vtpq_infer_min_score",
        default=0.0,
        type=float,
        help="Minimum normalized prune score used with --vtpq_infer_positive_only.",
    )
    parser.add_argument(
        "--vtpq_score_source",
        default="embed",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Prune visual tokens immediately before LLM prefill. "
            "Supported models: internvl2, qwen2_vl, llava_onevision."
        ),
    )
    parser.add_argument(
        "--vtpq_profile_prefill",
        action="store_true",
        help=(
            "Measure the first LLM forward and prompt KV cache during "
            "evaluation. Vision encoding is excluded; synchronized VTPQ "
            "scoring time is reported separately and added to the total."
        ),
    )
    parser.add_argument(
        "--vtpq_profile_warmup",
        default=10,
        type=int,
        help="Number of prefill calls to exclude as warmup.",
    )
    parser.add_argument(
        "--vtpq_profile_samples",
        default=200,
        type=int,
        help="Number of post-warmup prefill calls to profile.",
    )
    # for GPTQ
...
def cli_evaluate(args: Union[argparse.Namespace, None] = None) -> None:
    ...
    if is_main_process and results_list and all(
        result is None for result in results_list
    ):
        raise RuntimeError("All evaluation runs failed; no result JSON was written.")
...
def cli_evaluate_single(args: Union[argparse.Namespace, None] = None) -> None:
    ...
    if (
        ...
        and args.model.lower().replace("-", "_")
        not in VTPQ_INFERENCE_PRUNING_MODELS
    ):
        supported = ", ".join(sorted(VTPQ_INFERENCE_PRUNING_MODELS))
        raise NotImplementedError(
            "--vtpq_infer_prune does not support "
    ...
    if args.vtpq_profile_prefill:
        normalized_model = args.model.lower().replace("-", "_")
        if normalized_model not in PREFILL_PROFILE_MODELS:
            supported = ", ".join(sorted(PREFILL_PROFILE_MODELS))
            raise NotImplementedError(
                "--vtpq_profile_prefill does not support "
                f"--model {args.model}. Supported models: {supported}."
            )
        if str(args.batch_size) != "1":
            raise ValueError("--vtpq_profile_prefill requires --batch_size 1.")
        if args.vtpq_profile_warmup < 0:
            raise ValueError("--vtpq_profile_warmup must be non-negative.")
        if args.vtpq_profile_samples <= 0:
            raise ValueError("--vtpq_profile_samples must be positive.")
    ...
    raw_model = lm.model if hasattr(lm, "model") else lm._model
    vtpq_inference_stats = None
    if args.vtpq_infer_prune:
        vtpq_inference_stats = install_vtpq_inference_pruning(
        ...
        eval_logger.info(
            f"Installed {args.model} VTPQ inference pruning: "
            f"keep_ratio={args.vtpq_keep_ratio}, lambda={args.vtpq_lambda}, "
            f"score_bit={args.vtpq_score_bit}, "
            f"positive_only={args.vtpq_infer_positive_only}, "
            f"min_score={args.vtpq_infer_min_score}"
        )

    prefill_profiler_stats = None
    if args.vtpq_profile_prefill:
        prefill_profiler_stats = install_prefill_profiler(
            model_type=args.model,
            model=raw_model,
            warmup_samples=args.vtpq_profile_warmup,
            max_samples=args.vtpq_profile_samples,
        )
        eval_logger.info(
            f"Installed {args.model} prefill profiler: "
            f"warmup={args.vtpq_profile_warmup}, "
            f"samples={args.vtpq_profile_samples}"
        )
    ...
    if results is not None:
        ...
        if vtpq_inference_stats is not None:
            pruning_summary = vtpq_inference_stats.to_dict()
            results["vtpq_inference_pruning"] = pruning_summary
            eval_logger.info(
                "VTPQ inference pruning summary: "
                + json.dumps(pruning_summary, ensure_ascii=False)
            )

        if prefill_profiler_stats is not None:
            score_time_ms = (
                vtpq_inference_stats.mean_score_time_window_ms(
                    warmup_samples=prefill_profiler_stats.warmup_samples,
                    max_samples=prefill_profiler_stats.samples,
                )
                if vtpq_inference_stats is not None
                else 0.0
            )
            prefill_summary = prefill_profiler_stats.to_dict(
                mean_score_time_ms=score_time_ms
            )
            results["vtpq_prefill_profile"] = prefill_summary
            eval_logger.info(
                "VTPQ prefill profile summary: "
                + json.dumps(prefill_summary, ensure_ascii=False)
            )
